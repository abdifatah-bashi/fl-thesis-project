...
HeartDiseaseNet is composed of two independently deployed components that communicate exclusively via REST APIs. This separation ensures that the aggregation server never has access to patient data, and the hospital client never exposes its local training data to the network.
...
The backend is a stateless **FastAPI** application deployed on Google Cloud Run. It serves as the coordination layer for the entire federated network.
...
The genesis model is a **multi-layer perceptron (MLP)** built in PyTorch for binary heart disease classification:
...
<Steps>
  <Step>
    ...
    The server operator triggers `POST /api/model/publish`. The server creates a new PyTorch `HeartDiseaseNet`, transposes its weights to TF.js format, and stores them in memory as the **v1 global model**. The server reports `current_round: 1` via the status endpoint.
  </Step>
